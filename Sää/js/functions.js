...
const getLocation = () => {
    if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition(position => {
            ...
            getWeather(position.coords.latitude, position.coords.longitude) // Kutsutaan getWeather oikeilla parametreilla
        }, (error) => {
            alert(error)
        })
    } else {
...
const getWeather = (lat,lng) => {
    ...
    axios.get(address)
        .then(response => {
            ...
            description_span.innerHTML = json.weather[0].description // Muutetaan kuvaus oikein
            icon_img.src = icon_url + '/' + json.weather[0].icon + '@2x.png' // Korjataan ikonin URL
        })
        .catch(error => {
            alert(error)
        })
